...
# 17.12.2023: bams output just for the intermediate output without running sb2
rule all:
    input:
        ...
        config_yaml = "{wd}/{omics}/mags_generation.yaml".format(
                wd = working_dir,
                omics = config['omics']),
        # bams = expand("{wd}/{omics}/8-1-binning/depth_{scaf_type}/batch{batch}/{illumina}.{min_length}.bam",
        #         wd = working_dir,
        #         omics = config['omics'],
        #         scaf_type = SCAFFOLDS_type,
        #         batch = list(range(1, 1+batch_count[SCAFFOLDS_type[0]])),
        #         min_length = config['MIN_FASTA_LENGTH'],
...
rule map_contigs_BWA_depth_coverM:
    ...
    output:
        ...
        bam="{wd}/{omics}/8-1-binning/depth_{scaf_type}/batch{batch}/{illumina}.{min_length}.bam"
    shadow:
        "minimal"
    params:
...
rule combine_fasta_batches:
    ...
    shell:
        ...
        if [ "{params.multi}" == "yes" ]; then
            ...
            ln -s --force $(realpath {input[0]}) {output}
        fi
        """
...
# Generate csv files for SemiBin2 in the co-assembly mode
...
rule combine_input_data_batches:
    ...
    shell:
        """
        if [ "{params.multi}" == "yes" ]; then
            bash -c "cat {input.data[0]} | head -1" > combined.data
            (for file in {input.data}; do cat $file | tail -n +2; done) >> combined.data
            rsync -a combined.data {output.data}
            bash -c "cat {input.data_split[0]} | head -1" > combined.data_split
            (for file in {input.data_split}; do cat $file | tail -n +2; done) >> combined.data_split
            rsync -a combined.data_split {output.data_split}
        else
            ln -s $(realpath {input.data[0]}) {output.data}
            ln -s $(realpath {input.data_split[0]}) {output.data_split}
        fi
        """
